...
function init(){
	...
	function scoreboard(){
		...
		if(startup == true){
			...
			if(game == 'MBTL' || game == 'UNI2'){
				...
				TweenMax.set('#leftWrapper',{css:{y: adjust2}}); //if 'game' value is anything other than specified above it defaults to 2nd placement
	...
	function getData(){
		...
		if(startup == true){
			...
			TweenMax.to("#p1Country",.3,{css:{opacity: 1},ease:Quad.easeOut,delay:.3});
			TweenMax.to("#p2Country",.3,{css:{opacity: 1},ease:Quad.easeOut,delay:.3});
		}
		else{
			...
			if(countryHold1 != p1Country){
				TweenMax.to("#f1Wrapper",.3,{css:{opacity: 0},ease:Quad.easeOut,delay:0,onComplete:function(){
					$("#p1Country").attr("src","imgs/iso/64flat/"+p1Country+".png").on("error",function(){
						$("#p1Country").attr("src", "imgs/iso/64flat/unknown.png");
					});
					countryHold1 = p1Country;
					TweenMax.to("#f1Wrapper",.3,{css:{opacity: 1},ease:Quad.easeOut,delay:.2});
				}});
			}

			if(countryHold2 != p2Country){
				TweenMax.to("#f2Wrapper",.3,{css:{opacity: 0},ease:Quad.easeOut,delay:0,onComplete:function(){
					$("#p2Country").attr("src","imgs/iso/64flat/"+p2Country+".png").on("error",function(){
						$("#p2Country").attr("src", "imgs/iso/64flat/unknown.png");
					});
					countryHold2 = p2Country;
					TweenMax.to("#f2Wrapper",.3,{css:{opacity: 1},ease:Quad.easeOut,delay:.2});
				}});
			}
			...
			if(gameHold != game){ //checks to see if current json value for 'game' has changed from what is stored in gameHold html object
				...
				TweenMax.to('.logos',.3,{css:{opacity: 0},delay:0,onComplete:function(){ //then execute function
					...
					if(game == 'MBTL' || game == 'UNI2'){
						...
						TweenMax.set('#leftBGWrapper',{css:{y: '+0px', x: '0px'}});
						TweenMax.set('#rightBGWrapper',{css:{y: '+0px', x: '0px'}});
						TweenMax.set('#leftWrapper',{css:{y: adjust2, x:'0px'}});
						TweenMax.set('#rightWrapper',{css:{y: adjust2, x:'0px'}});
					}
					else if(game == 'GGST'){
